...
const ProductCard = ({ product }: ProductCardProps) => {
    const {id, image, title, price, author, rating } = product;
    return (
        <Link href={`/product/${id}`} className="flex flex-col gap-1 rounded-md bg-zinc-700/20 border border-white/10 transition-all ease-in-out duration-500 hover:scale-105" >
